...
const DonutChart = ({ data }) => {
  ...
  const segmentsWithAngles = data.reduce((acc, item, index) => {
    const percentage = (item.value / total) * 100;
    const angle = (percentage / 100) * 360;
    const startAngle = acc.length === 0 ? 0 : acc[acc.length - 1].endAngle;
    const endAngle = startAngle + angle;
    
    acc.push({ item, startAngle, endAngle, index });
    return acc;
  }, []);
  
  const renderedSegments = segmentsWithAngles.map(({ item, startAngle, endAngle, index }) => {
